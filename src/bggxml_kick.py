...
def parsegamedata(filenum, outfile):
    ...
    categories = [
        '|'.join(catlist) for catlist in
        [[category.attrib["value"] for category in
          categories] for categories in
         [game.findall(".//link[@type='boardgamecategory']") for game in
          root.findall("*")]]]

    mechanics = [
        '|'.join(mechlist) for mechlist in
        [[mechanic.attrib["value"] for mechanic in
          mechanics] for mechanics in
         [game.findall(".//link[@type='boardgamemechanic']") for game in
          root.findall("*")]]]

    families = [
        '|'.join(famlist) for famlist in
        [[family.attrib["value"] for family in
          families] for families in
         [game.findall(".//link[@type='boardgamefamily']") for game in
          root.findall("*")]]]

    designers = [
        '|'.join(deslist) for deslist in
        [[designer.attrib["value"] for designer in
          designers] for designers in
         [game.findall(".//link[@type='boardgamedesigner']") for game in
          root.findall("*")]]]
...
with open("bgg_games_20170530.csv", 'w') as csvout:
